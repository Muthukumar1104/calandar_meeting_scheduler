...
const CalendarScheduler = () => {
  ...
  const formattedEvents = Object.keys(groupedEvents).map((eventTime) => {
    ...
    return {
      ...
      meetingLink: firstEvent.meetingLink,
    };
  });
  ...
  const handleEventClick = (info) => {
    if (info?.event?._def?.extendedProps?.count > 1) {
      ...
      const get_meeting_link = formattedEvents?.find((event,i)=>event?.id == info?.event?._def?.publicId)?.meetingLink
      window.open(get_meeting_link, "_blank");
    }
  };
